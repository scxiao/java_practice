...
public class Alignment {
  ...
  public void matchString(float openGapPenalty,
    ...
    for (tid = 0; tid < threadNum; tid++) {
      t[tid] = new ThreadForMatchString(this, openGapPenalty,
                                        extGapPenalty, rowNum, columnNum,
                                        tileSize, tid, threadNum, cb);
      t[tid].start();
    }
  ...
  class ThreadForMatchString extends Thread {
    ...
    public void run() {
      ...
      int launchNo;
      for (launchNo = 0; launchNo < launchNum; launchNo++) {
        int threadOffset = threadIndex * tileSize;
        rowLoc = launchNo * tileSize - threadOffset + 1;
        for (columnLoc = threadOffset + 1; columnLoc < columnNum; columnLoc += stepSize) {
          if (rowLoc > 0 && rowLoc < rowNum) {
